...
999 continue ! no reagents file
  ...
  !*****************************************************************************
    ...
    ! Read the grid dimensions of the chemical field first, then allocate the
    ! reagent arrays to the chemical field's own size rather than the
    ! meteorological grid (nxmax/nymax/nzmax).  For a regional domain the met
    ! grid can be smaller than a global reagent field, in which case sizing the
    ! arrays with nxmax/nymax/nzmax caused out-of-bounds writes in nf90_get_var
    ! (heap corruption -> malloc abort).  Using max(met,chem) keeps runs whose
    ! met grid is the larger one byte-for-byte identical to before.
    ! NB: all reagents are assumed to share the same grid (arrays are allocated
    ! once, on the first reagent read).
    if (.not.allocated(nxCL)) then
    ...
    call nf90_err( nf90_inquire_dimension(ncid,dimid,len=nxCL(nr)) )
    call nf90_err( nf90_inq_dimid(ncid,'lat',dimid) )
    call nf90_err( nf90_inquire_dimension(ncid,dimid,len=nyCL(nr)) )
    call nf90_err( nf90_inq_dimid(ncid,'lev',dimid) )
    call nf90_err( nf90_inquire_dimension(ncid,dimid,len=nzCL(nr)) )

    if (.not.allocated(lonCL)) then
      allocate(lonCL(max(nxmax,nxCL(nr)),nreagent))
      allocate(latCL(max(nymax,nyCL(nr)),nreagent))
      allocate(altCL(max(nzmax,nzCL(nr)),nreagent))
      allocate(CL_field(max(nxmax,nxCL(nr)),max(nymax,nyCL(nr)),max(nzmax,nzCL(nr)),nreagent,2))
      allocate(clfield_cur(max(nxmax,nxCL(nr)),max(nymax,nyCL(nr)),max(nzmax,nzCL(nr)),nreagent))
    endif
...
!    close(600)
  ...
  !*****************************************************************************
    ...
    use point_mod,      only: xlon0, ylat0, dx, dy
    use windfields_mod, only: xresoln, yresoln, xln, yln, xrn, yrn, height, tt, prs, nz
    use omp_lib
    ...
    real                  :: clrate,temp
    real                  :: prsc,airnd,k0,kinf,rk
    real, parameter       :: smallnum = tiny(0.0) ! smallest number that can be handled
    ! Troe falloff parameters for OH + SO2 (JPL/IUPAC recommendation)
    real, parameter       :: fo_k0_300 = 3.3e-31  ! low-pressure limit at 300 K [cm^6 molecule^-2 s^-1]
    real, parameter       :: fo_k0_n   = -4.3     ! low-pressure temperature exponent [-]
    real, parameter       :: fo_kinf   = 1.6e-12  ! high-pressure limit [cm^3 molecule^-1 s^-1]
    real, parameter       :: fo_fc     = 0.6      ! broadening factor [-]
    real, parameter       :: kboltz    = 1.380649e-23 ! Boltzmann constant [J/K]
    real(kind=dp)         :: jul
    real                  :: lonjrx,latjry
#ifdef _OPENMP
...
!        if(ii.lt.100) print*, 'chemreaction: nr, clx, cly, clz, clzm = ',nr, clx, cly, clz, clzm
        ...
        if (cl_cur.gt.smallnum) then
          ...
          prsc=(prs(ix,jy,indz,1)*dtt2 &
                 + prs(ix,jy,indz,2)*dtt1)*dttt
          do ks=1,nspec
            if (reaccconst(nr,ks).gt.0..or.reacfalloff(nr,ks).gt.0) then
              if (reacfalloff(nr,ks).gt.0) then
                ! Pressure-dependent (Troe) falloff rate for termolecular
                ! reactions such as OH + SO2:
                ! k = (k0[M]/(1+k0[M]/kinf)) * Fc^(1/(1+(log10(k0[M]/kinf))^2)) [reagent]
                ! air number density [M] in molecules/cm^3 (prs in Pa)
                airnd=prsc/(kboltz*temp)*1.e-6
                k0=fo_k0_300*(temp/300.)**fo_k0_n
                kinf=fo_kinf
                rk=k0*airnd/kinf
                clrate=(k0*airnd/(1.+rk))*fo_fc**(1./(1.+(log10(rk))**2))*cl_cur
              else
