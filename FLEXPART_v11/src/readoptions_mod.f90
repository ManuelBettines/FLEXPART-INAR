...
subroutine readreleases
  ...
  if (any(reaccconst(:,:).gt.0.).or.any(reacfalloff(:,:).gt.0)) then
    CLREA=.true.
...
subroutine readspecies(id_spec,pos_spec)
  ...
  integer, allocatable, dimension(:) :: pfalloff
  ...
  namelist /species_params/ &
       ...
       preactions, pcconst, pdconst, pnconst, pfalloff, pohcconst, pohdconst, &
  ...
  if (.not.allocated(reaccconst)) then
    ...
    allocate(reacfalloff(maxreagent,nspec))
    reaccconst(:,:)=-9.99e-9
    reacdconst(:,:)=-9.99
    reacnconst(:,:)=-9.99
    reacfalloff(:,:)=0
    ...
    do j=1,maxreagent
      ...
      do i=1,maxreagent
        if (preactions(j).eq.reagents(i)) then
          ...
          reacfalloff(i,pos_spec)=pfalloff(j)
          exit
        endif
      end do
  ...
  if (lroot) then
    ...
    do j=1,nreagent
      ...
      write(*,*) reagents(j),': C, D, N, falloff = ',reaccconst(j,pos_spec),reacdconst(j,pos_spec), &
                 reacnconst(j,pos_spec),reacfalloff(j,pos_spec)
    end do
  endif
