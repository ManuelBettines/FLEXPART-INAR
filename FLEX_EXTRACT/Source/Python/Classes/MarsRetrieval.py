...
class MarsRetrieval(object):
    ...
    @staticmethod
    def _cds_param_id(param):
        '''Converts a MARS "paramID.table" parameter string into the plain
        GRIB paramID which the CDS API expects.

        The CDS adaptor of "reanalysis-era5-single-levels" does not resolve
        the "160.128" notation to a CDS variable any more. Such an entry is
        forwarded to MARS unchanged, which then fails with a MarsNoDataError
        for those dates where the field is not archived under exactly that
        date. The plain paramID ("160") is resolved by the adaptor and works.

        Parameters
        ----------
        param : str
            A single MARS parameter, either "paramID" or "paramID.table".

        Return
        ------
        str
            The parameter as plain GRIB paramID.

        '''
        if '.' not in param:
            return param
        number, table = param.split('.')
        if int(table) == 128:
            return str(int(number))
        return str(int(table) * 1000 + int(number))
    ...
    def _convert_to_cdsera5_sfc_request(self, attrs):
        ...
        newattrs['param'] = [self._cds_param_id(p)
                             for p in attrs['param'].split('/')]
        if '/' in attrs['step']:
